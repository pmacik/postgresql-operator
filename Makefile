...
# stable
OPERATOR_STABLE_VERSION ?= 0.0.9
# beta
OPERATOR_BETA_VERSION ?= 0.0.10
...
.PHONY: build-operator-image-beta
build-operator-image-beta: ./vendor refresh-tag get-tag
	operator-sdk build --image-builder buildah $(OPERATOR_IMAGE):$(OPERATOR_BETA_VERSION)-$(TAG)

.PHONY: push-operator-image-beta
push-operator-image-beta: get-tag
	@echo $(QUAY_PASSWORD) | podman login quay.io -u $(QUAY_USERNAME) --password-stdin
	podman push $(OPERATOR_IMAGE):$(OPERATOR_BETA_VERSION)-$(TAG)
...
deploy-operator-package: get-tag
	...
	cp -vf manifests/beta/* $(OPERATOR_MANIFESTS)
	@sed -i -e 's,REPLACE_NAME,$(OPERATOR_NAME),g' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	@sed -i -e 's,REPLACE_VERSION,$(OPERATOR_BETA_VERSION),g' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	@sed -i -e 's,REPLACE_TAG,$(TAG),g' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	@sed -i -e 's,REPLACE_IMAGE,$(OPERATOR_IMAGE):$(OPERATOR_BETA_VERSION)-$(TAG),g' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	@sed -i -e 's,REPLACE_CREATED_AT,$(CREATION_TIMESTAMP),' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	@sed -i -e 's,REPLACE_ICON_BASE64_DATA,$(ICON_BASE64_DATA),' $(OPERATOR_MANIFESTS)/postgresql-operator.v$(OPERATOR_BETA_VERSION).clusterserviceversion.yaml
	...
	@sed -i -e 's,REPLACE_BETA_VERSION,$(OPERATOR_BETA_VERSION),g' $(OPERATOR_MANIFESTS)/database.package.yaml
...
push-operator-package: get-tag
	...
	@operator-courier push $(OPERATOR_MANIFESTS) $(APPR_NAMESPACE) $(APPR_REPOSITORY) $(OPERATOR_BETA_VERSION)-$(TAG) "$(QUAY_API_TOKEN)"
...
.PHONY: beta-all
beta-all:	build-operator-image-beta push-operator-image-beta package-operator